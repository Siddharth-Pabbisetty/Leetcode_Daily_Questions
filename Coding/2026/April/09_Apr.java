...
class Solution {
    public int xorAfterQueries(int[] nums, int[][] queries) {
        ...
        ArrayList<int[]>[][] ls=new ArrayList[c+1][c+1];

        for(int i=0;i<c;i++)
        {
            for(int j=0;j<c;j++)
            {
                ls[i][j]=new ArrayList<>();
            }
        }
        for(int i=0;i<m;i++)
        {
        ...
        for(int k=1;k<c;k++)
        {
            ...
            {
                if(ls[k][rem].isEmpty()) continue;
                ArrayList<int[]> queriesList = ls[k][rem];
