...
class Solution {
    ...
    public long maxTotalValue(int[] nums, int k) {
        ...
        for (int l = 0; l < n; l++)
        {
            pq.offer(new Node(getValue(l, n - 1, mx, mn, arr), l, n - 1));
        }
        long ans = 0;
        while (k-- > 0)
        {
            ...
            if (cur.r > cur.l)
            {
                pq.offer(new Node(getValue(cur.l, cur.r - 1, mx, mn, arr), cur.l, cur.r - 1));
            }
        }
        return ans;
    }

    private long getValue(int l, int r, int[][] mx, int[][] mn, int[] arr) {
        int len=r-l+1;
        int p=arr[len];
